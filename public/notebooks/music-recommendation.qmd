...
**Identifier encoding:** User and song identifiers are label-encoded into numeric ids, improving model runtime without changing the underlying interaction signal.
...
**Interaction filtering:** The matrix retains active users and frequently listened songs to reduce sparsity, keep computation manageable, and improve recommendation quality.
...
**Distribution check:** The play-count distribution is inspected to understand interaction imbalance before modeling.
...
**Evaluation frame:** Models are compared with RMSE, precision@k, recall@k and F1@k, covering both rating error and recommendation relevance.
...
**Threshold rationale:** A 1.5 threshold separates weak listening signals from relevant recommendations. Lower values over-recommend low-signal songs; higher values make the system too conservative. F1 balances precision and recall.
...
**Split sensitivity:** Because interactions are imbalanced, the test split affects how much listening diversity appears in evaluation. A larger test set can expose more diverse behavior, while a smaller one may make metrics less representative.
...
**Similarity inspection:** The tuned model returns nearest-neighbor songs, making item similarity easier to inspect qualitatively.
...
**Ranking correction:** The `1 / sqrt(n)` adjustment tempers overconfidence on very frequent songs. Subtracting it produces a conservative score; adding it would be more optimistic.
...
**KNN tuning:** Similarity options and neighbor count drive KNN performance. Grid search selects the strongest configuration.
...
**SVD tuning:** Latent-factor settings such as learning rate, regularization and epochs can improve prediction quality compared with the default model.
...
**CoClustering tuning:** Cluster counts and training epochs affect CoClustering quality. Grid search is used to improve the baseline configuration.
...
**Content features:** Song metadata extends the signal beyond play counts, enabling recommendations based on similar titles, releases and artists.
