...
### **The Context:**
 ...
 - Nowadays we are overloaded by information. Selecting a qualitative content over this huge amount of information has become difficult more than ever. But companies understood this problem we are facing, so they are coming up with technics to help us choose the best content and improve the overall quality of the customer journey on their platforms. One of those companies is Spotify. Spotify has became an industry leader in music streaming partly due to the recommendation systems they built to help their customers discover new content they might like.
 - So recommendation systems play a crucial role in our lives as consumers and also for companies which intend to deliver a great service and engage the customers on their plateforms.   

### **The objective:**

 - The goal of this project is to build a recommendation system to propose the top 10 songs for a user based on the likelyhood of listening to those songs. We will use two datasets:

    - The dataset containing the play counts of the songs for each user and
    - The dataset containing the descriptions of the songs.  


### **The key questions:**

- During this project we will try to answer to some key questions:
  - What are the possible solutions for this problem ? Do we need modeling, if yes which kind of model do we need ?
  - Do we have enough data to build a recommendation system ?
  - Does our data contain all the information we need to build a recommendation system ?
  - What are the most important metrics for the business ? Some key metrics are : precision, recall, F1 score, interpretability, responsiveness, ethical considerations.
  - Are our models good enough for productionalizing ? How does the quality of the recomendations behave over time ?
  - What are the next steps ?

### **The problem formulation**:

- Given the songs data and the count data, we are trying to predict for each user which songs he would like the most.
- The proxy we will use to find if a user would like a song or not is the predicted rating for that song. A rating threshlod will be used. We will recommend a song to a user if the predicted rating is above that threshold. We will also predict a list of K songs that the user would like the most among all the songs.
- We will build various recommendation systems:
  - Knowledge/Rank based recommendation system
  - Similarity-Based recommendation systems:
    - clustering based recommendation system
    - user-user collaborative filtering system
    - item-item collaborative filtering system
  - Matrix Factorization based collaborative filtering system
