...
const buildImagePath = (folder, section, file) =>
  `/${folder}/${section}/${file}`.replace(/\/+/g, '/')
...
export const ExplodedBookGallery = ({
  ...
  authorDescription,
  books = [],
  folder = 'images/blog/readings/books-changed-my-life',
  section = '',
}) => {
  const authorSrc = buildImagePath(folder, section, authorImage)
  ...
  return (
    <section className="space-y-16 px-4 sm:px-6">
      ...
      <div className="space-y-20">
        {books.map((book, idx) => {
          const bookSrc = buildImagePath(folder, section, book.image)
